...
The `_base/php-fpm` directory builds the base image from the Ubuntu 26.04 LTS image along with Cerb's required PHP extensions.
...
```bash
docker build -t cerb:base-8.5-fpm --platform=linux/arm64,linux/amd64 .
```
